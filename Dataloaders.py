...
class raw_to_depth(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.from_numpy(left_raw).float(),
            torch.from_numpy(right_raw).float(),
...
class raw_spec_to_depth(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.from_numpy(left_spec).float(),
            torch.from_numpy(right_spec).float(),
            torch.from_numpy(left_raw).float(),
            torch.from_numpy(right_raw).float(),
            torch.from_numpy(depth).float(),
        )
...
class raw_to_image(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = np.expand_dims((np.array(image) * 1 / 255), axis=-1).transpose(
            (2, 0, 1)
        )
